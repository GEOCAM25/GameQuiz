// ============================================================
// GAME QUIZ — ¿QUIÉN SERÁ? 🫵  (¿Quién es más probable que...?)
// Juego grande para todo el grupo: sale una pregunta ("¿Quién es más
// probable que...?") y TODOS votan por la persona que mejor calza.
// Cada voto recibido suma 10 puntos; al terminar la partida, quien
// tenga MÁS puntos (más votos) PIERDE 💀. Al cerrar cada pregunta se
// muestra quién votó por quién (o solo el conteo si el anfitrión eligió
// votos anónimos antes de empezar).
//
...
// atrevido (picante). Partidas de 15/20/25 preguntas. Votos anónimos o
// a la vista.
// ============================================================
const Mojate = (() => {
  ...
  function renderStart(){
    ...
    host.innerHTML = `
      <div class="imp-wrap">
        ...
        <h2 class="imp-title">¿Quién será?</h2>
        <p class="imp-tag">¿Quién es más probable que…? Todos votan. Cada voto = 10 puntos… ¡y quien más junte al final, PIERDE! 😬</p>
  ...
  function hostDispatch(cmd){
    ...
    switch (cmd.t){
      ...
      case "setAnon": if(f.phase==="lobby") f.anon=!!cmd.v; break;
  ...
  function hostReveal(){
    ...
    f.result = { question:f.question, tally, winners, last: f.round >= f.total, anon: f.anon, votes: f.anon ? null : f.votes };
    f.phase = "reveal";
    broadcastPub();
  }
  function broadcastPub(){
    const f = S.full; if (!f) return;
    const pub = { code:f.code, leader:f.leader, phase:f.phase, mode:f.mode, total:f.total, anon:f.anon, players:f.players, order:f.order,
  ...
  function renderLobby(p){
    ...
    if(amLeader){
      ...
      $$("#mojAnon button").forEach(b=>b.onclick=()=>send0({t:"setAnon",v:b.dataset.v==="1"}));
  ...
  function renderReveal(p){
    ...
    const last = !!r.last, anon = !!r.anon;
    const names = id => p.players[id]?.name || "?";
    const winnersTxt = (r.winners||[]).map(names).join(" y ");
    const roundPts = (r.winners&&r.winners.length && r.tally) ? ((r.tally[r.winners[0]]||0)*10) : 0;
    // quién votó a quién (solo si NO es anónimo)
    const byTarget = {};
    if(!anon) Object.entries(r.votes||{}).forEach(([voter,target]) => { (byTarget[target] = byTarget[target]||[]).push(voter); });
    // puntos = votos recibidos en la noche × 10. Quien más tenga, PIERDE.
    const ranking = p.order.map(id=>({id,name:names(id),pts:(p.players[id]?.noms||0)*10})).sort((a,b)=>b.pts-a.pts);
    const maxPts = ranking.length ? ranking[0].pts : 0;
    const rankRows = ranking.map((x,i)=>{
      const loser = last && x.pts>0 && x.pts===maxPts;
      return `<div class="imp-brow ${x.id===S.myId?"me":""} ${loser?"moj-loser":""}"><span>${loser?"💀":i+1+"º"}</span><span class="imp-bnm">${esc(x.name)}</span><span>${x.pts} pts</span></div>`;
    }).join("");
    const loserTxt = maxPts>0 ? ranking.filter(x=>x.pts===maxPts).map(x=>x.name).join(" y ") : "Nadie";
    host.innerHTML = `
      <div class="imp-wrap">
        ${top(p, `Ronda ${p.round}/${p.total}`)}
        ...
        <div class="moj-winner">
          ...
          <p class="moj-wsub">${roundPts>0 ? `+${roundPts} puntos${(r.winners||[]).length>1?" cada uno":""} 😬` : "Sin votos"}</p>
        </div>
        <div class="moj-breakdown">
          ${p.order.map(id=>{
            const c = (r.tally&&r.tally[id])||0;
            const voters = (byTarget[id]||[]).map(names).join(", ");
            return c>0 ? `<div class="moj-brow"><span class="moj-bn">${esc(names(id))}</span><span class="moj-bc">${c*10} pts</span><span class="moj-bv">${anon?`${c} 🗳️`:esc(voters)}</span></div>` : "";
          }).join("")}
        </div>
        ${ last ? `
          <div class="moj-final">
            <p class="moj-final-t">🏁 Fin de la partida</p>
            <p class="moj-final-champ">💀 ¡Pierde <b>${esc(loserTxt)}</b> con ${maxPts} pts!</p>
        ...
        ` : `
          <details class="moj-rank"><summary>📊 Van perdiendo (puntos acumulados)</summary>${rankRows}</details>
